...
# --- 1. 数据加载与预处理 ---
# 尝试加载数据集
print("Loading TOXcast dataset...")

# Corrected: Use MoleculeNet to load ToxCast
dataset = MoleculeNet(root='./data/Toxcast', name='ToxCast')
...
val_size = int(0.2 * len(filtered_dataset))
...
#     model_save_path = 'results/toxcast/multilevel_gnn_model' + f'_epoch{epoch}' + '.pth'
#     torch.save(model.state_dict(), model_save_path)
#     print(f"model saved to {model_save_path}")

all_results = []
for id in range(100, 101):
    model_save_path = f'results/toxcast/multilevel_gnn_model_epoch{id}.pth'
    ...
    for task_idx in range(NUM_TASKS):
        ...
        if current_task_name in test_auc_dict:
            task_results.append(test_auc_dict[current_task_name])
    
    all_results.append(task_results)

with open('results/toxcast/multilevel_gnn_results_100.txt', 'w') as f:
    for i, result in enumerate(all_results):
        f.write(f"{100 + i},{','.join(map(str, result))}\n")
    print("Results saved to results/toxcast/multilevel_gnn_results.txt")